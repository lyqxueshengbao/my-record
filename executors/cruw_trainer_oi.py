...
class CruwExecutorOI(CruwExecutor):
    ...
    def validation_step(self, batch, batch_id):
        ...
        """
        # --- 在开头添加 ---
        if self.val_frame_counter % self.win_size == 0:
            self.model.encoder.__init_hidden__()
        self.val_frame_counter += 1
        # --- 添加结束 ---
        # Get data
        ra_maps = batch['radar_data']  # N, H, W, C
        confmap_gts = batch['anno']['confmaps']
    ...
    def test_step(self, batch, batch_id):
        ...
        """
        # --- 在开头添加 ---
        if self.test_frame_counter % self.win_size == 0:
            self.model.encoder.__init_hidden__()
    ...
    def on_validation_start(self):
        self.model.encoder.__init_hidden__()
        self.val_frame_counter = 0  # 添加此行

    def on_test_start(self):
        self.model.encoder.__init_hidden__()
        self.test_frame_counter = 0  # 添加此行
