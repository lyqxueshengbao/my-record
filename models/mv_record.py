...
from utils import get_norm_layer  # <--- MODIFIED: 修复了循环导入


class MVRecord(nn.Module):
    def __init__(self, config, n_classes, n_frames, in_channels, norm):
        super(MVRecord, self).__init__()
        self.config = config.model_config
        self.n_classes = n_classes
        self.n_frames = n_frames
        self.norm_layer = get_norm_layer(norm)
        self.ra_encoder = RECORD(config=config, in_channels=in_channels, norm=norm, n_class=n_classes)
        self.rd_encoder = RECORD(config=config, in_channels=in_channels, norm=norm, n_class=n_classes)
        self.ad_encoder = RECORD(config=config, in_channels=in_channels, norm=norm, n_class=n_classes)

        self.conv_t1_ra = nn.ConvTranspose2d(self.ra_encoder.channels[7] * 3,
                                             self.ra_encoder.channels[5], 2, 2)

        # <--- MODIFIED BLOCK START --->
        hidden_dim_ra_1 = int(round(self.ra_encoder.channels[5] * 4))  # expand_ratio = 4
        self.conv9_ra = GhostBottleneck(
            in_chs=self.ra_encoder.channels[5],
            mid_chs=hidden_dim_ra_1,
        ...
        # <--- MODIFIED BLOCK START --->
        hidden_dim_rd_1 = int(round(self.rd_encoder.channels[5] * 4))  # expand_ratio = 4
        self.conv9_rd = GhostBottleneck(
            in_chs=self.rd_encoder.channels[5],
            mid_chs=hidden_dim_rd_1,
    ...
    def forward(self, x, states):
        ...
        states_ra, states_rd, states_ad = states
        (h1_ra, c1_ra), (h2_ra, c2_ra) = states_ra
        (h1_rd, c1_rd), (h2_rd, c2_rd) = states_rd
        (h1_ad, c1_ad), (h2_ad, c2_ad) = states_ad
        ...
        states_ra = ((h1_ra, c1_ra), (h2_ra, c2_ra))
        states_rd = ((h1_rd, c1_rd), (h2_rd, c2_rd))
        states_ad = ((h1_ad, c1_ad), (h2_ad, c2_ad))
        return (x_ra, x_rd), (states_ra, states_rd, states_ad)