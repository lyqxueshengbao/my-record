...
class RECORD_OI(nn.Module):

    def __init__(self, config):
        super(RECORD_OI, self).__init__()
        ...
        # conv1
        hidden_dim_0 = int(round(self.in_channels * 1))  # expand_ratio = 1
    ...
    def init_states(self, bsize):
        h1 = torch.zeros(bsize, self.channels[3], self.config['input_size'][0] // 4,
                         self.config['input_size'][1] // 4,
                         device=self.conv1.conv_dw.weight.device)
        c1 = torch.zeros(bsize, self.channels[3], self.config['input_size'][0] // 4,
                         self.config['input_size'][1] // 4,
                         device=self.conv1.conv_dw.weight.device)
        h2 = torch.zeros(bsize, self.channels[5], self.config['input_size'][0] // 8,
                         self.config['input_size'][1] // 8,
                         device=self.conv1.conv_dw.weight.device)
        c2 = torch.zeros(bsize, self.channels[5], self.config['input_size'][0] // 8,
                         self.config['input_size'][1] // 8,
                         device=self.conv1.conv_dw.weight.device)
        return ((h1, c1), (h2, c2))
    ...
    def forward(self, x):
        ...
        # head
        x = self.head(x)
        self.states = ((h1.detach(), c1.detach()), (h2.detach(), c2.detach()))
        return x