...
class BottleneckLSTMCell(nn.Module):
    def __init__(self, input_channels, hidden_channels, kernel_size=3, norm='layer', num_groups=8):
        """
        ...
        @param norm: normalisation to use on output gates (default: 'layer' uses GN)
        ...
        if norm is not None:
            ...
            def _get_norm(channels):
                effective_num_groups = min(self.num_groups, channels)
                while channels % effective_num_groups != 0:
                    effective_num_groups -= 1
                return nn.GroupNorm(effective_num_groups, channels)

            self.norm_wbi = _get_norm(self.hidden_channels)
            self.norm_wbf = _get_norm(self.hidden_channels)
            self.norm_wbc = _get_norm(self.hidden_channels)
            self.norm_wbo = _get_norm(self.hidden_channels)
...
class BottleneckLSTM(nn.Module):
    def __init__(self, input_channels, hidden_channels, norm='layer', num_groups=8):
        """
        ...
        @param norm: normalisation to use (default: 'layer' uses GN)
        @param num_groups: number of groups for GroupNorm (default: 8)
        """
        ...
        self.cell = BottleneckLSTMCell(self.input_channels, self.hidden_channels, norm=norm, num_groups=num_groups)
